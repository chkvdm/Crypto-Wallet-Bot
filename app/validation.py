...
class Validation():
    ...
    def tg_user_id_validate(self, tg_user_id):
        ...
        if re.fullmatch('[0-9]{9}', str(tg_user_id)) == None:
            abort (400, description = f"invalid telegram ID")
        # validation of availability a telegram ID 
        if not (session.query(Profile).filter_by(tg_user_id = str(tg_user_id))
                .first()):
            return None
        else:
            abort(
                403, 
                description = f"The telegram ID '{tg_user_id}' is alredy exist"
                )

    def name_validate(self, name):
        # validation name format
        if re.fullmatch ('[0-9A-Za-z_]{5,}$', name) == None:
