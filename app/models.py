...
engine = create_engine(
                'postgresql+psycopg2://postgres:example@db:5432/CryptoBotDB'
                )
...
class Transaction():
    def __init__(self, profile_id, timestamp, currency_id, amount, 
                transaction_type_id):
        self.profile_id = profile_id
...
class TransactionSchema(ma.Schema):
    class Meta:
        fields = (
            'profile_id', 'timestamp', 'currency_id', 'amount', 
            'transaction_type_id'
            )
