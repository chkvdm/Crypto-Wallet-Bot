...
class Wallet():
    ...
    def hello_user_balance(self, tg_user_id):
        # uuid - the value of the field 'id' that is automatically assigned to 
        # the user at the time of registration. For convenience, in all 
        # functions, we make the value of the uuid field equal to the 
        # variable 'profile_id'.
        profile_id = (session.query(Profile).filter_by(tg_user_id = 
                                                    str(tg_user_id)).one()).id
        if not profile_id:
            abort (404, description = "User not found")
        # welcome benefit = 10000.0 USDT
        hello_usdt_bonus = 10000.0
        # welcome benefit money transfer
        hello_usdt_bonus_top_up = Balance(profile_id, (session.query(
                                    Currency).filter_by(
                                        name = 'USDT').one()).id, 
                                        hello_usdt_bonus)
        session.add(hello_usdt_bonus_top_up)
        # welcome benefit money transfer log
        hello_transaction_log = Transaction(
                                    profile_id, 
                                    datetime.now(timezone.utc).isoformat(), 
                                    (session.query(Currency).filter_by(
                                        name = 'USDT').one()).id, 
                                        hello_usdt_bonus, 
                                        (session.query(
                                            Transaction_type).filter_by(name =
                                             'hello bonus').one()).id)
    ...
    def user_balance(self, tg_user_id):
        ...
        user_balance_list = session.query(Balance.total, 
                                    Currency.name).join(Currency, 
                                        Balance.currency_id == 
                                        Currency.id).filter(Balance.profile_id 
                                            == profile_id.id).all()
    ...
    def user_transaction(self, tg_user_id):
        ...
        if not profile_id:
            abort (404, description = f"User not found")
        user_transaction_list = session.query(Transaction.timestamp, 
                    Transaction.amount, Currency.name.label("currency"), 
                    Transaction_type.name).join(Currency, 
                    Transaction.currency_id == Currency.id).join(
                        Transaction_type, 
                        Transaction.transaction_type_id == 
                        Transaction_type.id).filter(
                            Transaction.profile_id == 
                            profile_id.id).order_by(
    ...
    def swap(currency_sell_id, currency_buy_id, amount_currency_sell, 
                                        ticker_sell, ticker_buy, profile_id):
        # sele transaction
        sell_on_balance = session.query(Balance).filter_by(profile_id = 
                            profile_id, currency_id = currency_sell_id).first()
        sell_on_balance.total -= Decimal(amount_currency_sell)
        # sale transaction log
        sell_data_log = Transaction(profile_id, 
                            datetime.now(timezone.utc).isoformat(), 
                            currency_sell_id, amount_currency_sell, 
                            (session.query(Transaction_type).filter_by(name = 
                                'sell').one()).id)
        session.add(sell_data_log)
        # buy transaction
        buy_on_balance = session.query(Balance).filter_by(
                                        profile_id = profile_id, 
                                        currency_id = currency_buy_id).first()
        buy_on_balance.total += Decimal(amount_currency_sell)*\
                                Wallet.exchange_rate(ticker_sell, ticker_buy)
        # buy transaction log
        buy_data_log = Transaction(profile_id, 
                            datetime.now(timezone.utc).isoformat(), 
                            currency_buy_id, 
                            Decimal(amount_currency_sell)*\
                                Wallet.exchange_rate(ticker_sell, ticker_buy), 
                            (session.query(Transaction_type).filter_by(name = 
                                                            'buy').one()).id)
    ...
    # checking the currency the user is selling
    
    def currency_sell_control(currency_sell_id, ticker_buy, ticker_sell, 
                                            amount_currency_buy, profile_id):
        # check availability currency which sell on balance
        check_currency_sell = session.query(Balance).filter_by(profile_id = 
                            profile_id, currency_id = currency_sell_id).first()
        if not check_currency_sell:
            abort(404, description = f"You do not have {ticker_sell}")
        # validation amount
        if check_currency_sell.total < Decimal(amount_currency_buy)/\
                                Wallet.exchange_rate(ticker_sell, ticker_buy):
            abort(404, description = f"You do not have enough {ticker_sell}")

    def buy(self, tg_user_id, trading_information):
        user_info = session.query(Profile).filter_by(tg_user_id = 
                                                    str(tg_user_id)).first()
        if not user_info:
            abort(404, description = "user not found")
        ...
        currency_buy_id = (session.query(Currency).filter_by(name = 
                                                        ticker_buy).one()).id
        currency_sell_id = (session.query(Currency).filter_by(name = 
                                                        ticker_sell).one()).id
        profile_id = (session.query(Profile).filter_by(tg_user_id = 
                                                        tg_user_id).one()).id
        # validation availability currency which sell on balance
        Wallet.currency_sell_control(currency_sell_id, ticker_buy, ticker_sell,
                                             amount_currency_buy, profile_id)
        # check field with currency which we buy in table 'balance' in data base
        currency_buy_in_wallet = session.query(Balance).filter_by(profile_id = 
                            profile_id, currency_id = currency_buy_id).first()
        if not currency_buy_in_wallet:
            ...
            session.commit()
        amount_currency_sell = Decimal(amount_currency_buy)/\
                                Wallet.exchange_rate(ticker_sell, ticker_buy)
        Wallet.swap(currency_sell_id, currency_buy_id, amount_currency_sell, 
                                        ticker_sell, ticker_buy, profile_id)
    ...
    def sell(self, tg_user_id, trading_information):
        ...
        CurrencyInfo().check_currency_support(ticker_buy)
        CurrencyInfo().check_currency_support(ticker_sell)
        amount_currency_buy = Decimal(amount_currency_sell)*\
                                Wallet.exchange_rate(ticker_sell, ticker_buy)
